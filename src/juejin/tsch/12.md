...
在递归那节我们写过取 Promise 的 ValueType 的高级类型，这个比较常用，ts 也给内置了，就是 Awaited。
...
继续提取 onfulfilled 函数类型的第一个参数的类型，也就是 Promise 返回的值的类型到 infer 声明的局部变量 V。
...
then 第一个参数是 onfulfilled 的回调，从它的第一个参数就能拿到返回的值的类型。
